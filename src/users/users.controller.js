...
const createUserController = async (req,res) => {
    ...
    if(!username || !name || !email || !password || !avatar){//if you don't have any of these
        //400 status is bad reuest
        return res.status(400).send({message: "alguns campos precisam ser preenchidos!",});
    };
...
if(foundUser){//if there exist any user that comes from service
 return res.status(400).send({// erro number 400 is bad request
